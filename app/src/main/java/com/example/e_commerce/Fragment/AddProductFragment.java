        ...
        btn_add.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(!image.isEmpty() && !name.isEmpty() && price!=-1 && quantiry!=-1){
                    for (int i = 0; i < categories.size(); i++) {
                        // Log.d("AddProductFragment", "Category[" + i + "]: " + categories.get(i).getName());
                        if (categories.get(i).getName().trim().equalsIgnoreCase(category)) {
                            cat_id = categories.get(i).getId();
                            // Log.d("AddProductFragment", "Match found. Category ID: " + categories.get(i));
                            break;
                        }
                    }
